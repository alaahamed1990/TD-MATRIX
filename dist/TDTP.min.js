...
class Document
{constructor()
{this.Value="";this.terms={};this.length=0;this.SetIndex=0;}
get Color()
{var value=this.SetIndex;value>>>=0;var b=value&0xFF,g=(value&0xFF00)>>>8,r=(value&0xFF0000)>>>16,a=((value&0xFF000000)>>>24)/ 255;return"rgba("+[r,g,b,1].join(",")+")";}
...
class Term
{constructor(value)
{this.value=value;this.Frequency=0;this.Weight=0;this.DocumentFreq=null;}}
...
{var document=new Document();document.Value=quotes[index];var tokens=this.ExtractTokens(quotes[index],document,docFrequencies);result.push(document);}
...
{sentences[sentenceIndex]=sentences[sentenceIndex].trim();if(!IsDefined(sentences[sentenceIndex]))
{continue;}
var isFirstTokenNumber=sentences[sentenceIndex].match(/^\d+\s+/)!=null;if(isEndOfSentence||(isLastTokenNumber&&!isFirstTokenNumber))
{tmpDocument=new Document();isEndOfSentence=true;isLastTokenNumber=false;}
lastToken=this.ExtractTokens(sentences[sentenceIndex],tmpDocument,docFrequencies);isLastTokenNumber=IsTokenNumber(lastToken);if(!abbrevations[lastToken]&&!isLastTokenNumber)
{tmpDocument.SetIndex=counter++;resultSentences.push(tmpDocument);isEndOfSentence=true;}
else{isEndOfSentence=false;}}
return resultSentences;};ExtractTokens(quote,document,docFrequencies)
{if(!IsDefined(document))
{document=new Document();}
var tokens=quote.split(tokenDelimeters);var isFirstTokenNumber=IsTokenNumber(tokens[0]);document.Value+=quote;var lastToken=null;for(var tokenIndex in tokens)
{var currentToken=lastToken=tokens[tokenIndex];if(!IsDefined(currentToken)||stopWords[currentToken])
{continue;}
currentToken=memoizingStemmer(currentToken).toLowerCase();if(!IsDefined(document.terms[currentToken]))
{document.terms[currentToken]=new Term();document.terms[currentToken].value=currentToken;if(!IsDefined(docFrequencies[currentToken]))
{document.terms[currentToken].DocumentFreq=docFrequencies[currentToken]={value:0};}
else if(!IsDefined(document.terms[currentToken].DocumentFreq)){document.terms[currentToken].DocumentFreq=docFrequencies[currentToken];}
docFrequencies[currentToken].value+=1;}
document.terms[currentToken].Frequency++;document.length++;}
return lastToken;}
ApplyTf(documents,minTermWeight)
{for(var documentIndex in documents)
{var currentDocument=documents[documentIndex];for(var termIndex in currentDocument.terms)
{var currentTerm=currentDocument.terms[termIndex];currentTerm.Weight=currentTerm.Frequency/currentDocument.length;if(currentTerm.Weight<minTermWeight)
{delete currentDocument.terms[termIndex];}}}}
ApplyTfIDf(sentences,minTermWeight)
{var totalNumberOfDocs=sentences.length;for(var statmentIndex in sentences)
{var currentDocument=sentences[statmentIndex];for(var termIndex in currentDocument.terms)
